...
class MockProvider(Provider):
    ...
    def ask(
        ...
        payload = AdvisorPayload(
            ...
            plan_markdown=self._plan_markdown(snippet) if mode == "plan" else "",
    ...
    def _plan_markdown(self, snippet: str) -> str:
        provider_reason = {
            "codex": "codex — 複数ファイルにまたがる実装順の整理と実装着手に向いているため。",
            "claude": "claude — 文章品質と制約整理が重要なタスクに向いているため。",
            "gemini": "gemini — 周辺文脈の整理や補助的な調査タスクに向いているため。",
        }.get(self.name, f"{self.name} — この計画の主担当として扱うため。")
        return "\n".join(
            [
                f"# Plan: {self.name} による実装計画",
                "",
                "## Objective",
                f"{snippet} を、実装順と検証条件がぶれない形で段階的に進める。",
                "",
                "## Background",
                "- 現在の MAGI は plan.md を計画の正式成果物として扱う。",
                "- 実装は別の CLI に handoff する前提で、各タスクに担当候補と完了条件を含める。",
                "- run の記録と report.md は継続して残す必要がある。",
                "",
                "## Tasks",
                "",
                "### Task 1: plan 出力仕様を固定する",
                "",
                "**Scope**: plan.md の構造、必須セクション、禁止表現を固定し、後続の実装が同じ粒度で進むようにする。",
                "",
                "**Non-goals**:",
                "- 実装そのものには着手しない。",
                "",
                "**Files**:",
                "- `docs/plan-style-guide.md` — plan の記述規約を定義する。",
                "- `magi/prompts/plan_format.md` — prompt から参照する簡潔な出力契約を置く。",
                "",
                "**Risks**:",
                "- ルールが抽象的すぎると provider ごとの出力差が残る。Mitigation: 禁止表現と必須項目を明記する。",
                "",
                "**Done when**:",
                "- [ ] `docs/plan-style-guide.md` に必須セクションと task template が明記されている。",
                "- [ ] `magi/prompts/plan_format.md` に provider が従う出力契約がまとまっている。",
                "",
                f"**Suggested provider**: {provider_reason}",
                "",
                "**Handoff prompt**:",
                "> `docs/plan-style-guide.md` と `magi/prompts/plan_format.md` を更新し、plan.md の必須セクション、task template、禁止表現、handoff prompt の書式を明記してください。完了条件は、両ファイルに同じルールが矛盾なく反映されていることです。",
                "",
                "### Task 2: plan.md 生成処理をテンプレート準拠にする",
                "",
                "**Scope**: report.md の包み直しではなく、plan 用の構造化 Markdown を `plan.md` に書き出す。",
                "",
                "**Non-goals**:",
                "- ask / debug / agent の report 形式は変更しない。",
                "",
                "**Files**:",
                "- `magi/pipeline.py` — plan.md の生成処理を更新する。",
                "- `magi/providers/external_cli.py` — provider から plan 専用 Markdown を受け取れるようにする。",
                "",
                "**Risks**:",
                "- 外部 provider が新しい plan フィールドを返さない場合に plan.md が崩れる。Mitigation: fallback の plan 生成を残す。",
                "",
                "**Done when**:",
                "- [ ] `plan.md` が `# Plan:` から始まり、`## Objective`, `## Background`, `## Tasks`, `## Open Questions`, `## Out of Scope` を含む。",
                "- [ ] provider が plan 専用フィールドを返さなくても plan.md が空にならない。",
                "",
                f"**Suggested provider**: {provider_reason}",
                "",
                "**Handoff prompt**:",
                "> `magi/pipeline.py` と `magi/providers/external_cli.py` を更新し、plan モードでは provider の構造化 Markdown を `plan.md` に採用してください。`plan.md` は `# Plan:` から始まり、必須セクションをすべて含めてください。provider が新フィールドを返さない場合は fallback で plan を生成してください。",
                "",
                "## Open Questions",
                "- 実運用で `plan.md` を常に日本語固定にするかは、利用者のワークフローで確認が必要。",
                "",
                "## Out of Scope",
                "- 複数 provider の plan を自動統合して 1 本の task 列にマージする処理。",
            ]
        )
